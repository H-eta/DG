...
void draw() {
  ...
  if (curtainAnimationEnded) {
    if (!floor_disappear_flag){
      ...
      if (millis() - light_timer > 11000 && millis() - light_timer < 85000) {  // give some time for things to be positioned)
        lightVisible = true;
      ...
      if (millis() - light_timer >= 85000){  // foi a única forma que arranjei para que este porcaria ficasse visível durante um tempo e depois desaparecesse
        lightVisible = false;
      }
    ...
    for (int i = 0; i < numImages - 1; i++) {
      for (int j = i + 1; j < numImages; j++) {
        ...
        if (checkOverlap(rand_imageX[i], rand_imageY[i], images[i].width * 0.8, images[i].height * 0.8, 
          ...
          float offsetY = 0.02; // Move image i by its height down
          ...
          //if (rand_imageY[i] + offsetY >= displayHeight) {
